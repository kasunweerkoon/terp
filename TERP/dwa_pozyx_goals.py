...
class Config():
    ...
    def assignOdomCoords(self, msg):
        ...
        # (roll,pitch,theta) = euler_from_quaternion ([rot_q.z, -rot_q.x, -rot_q.y, rot_q.w])
        self.th = theta
        # print("roll = ", roll)
        # print("pitch = ", pitch)
        # print("Theta = ", self.th)
    ...
    def getEnvState(self,data):
        if data.linear.x == 100:
            data_pos = rospy.wait_for_message('/target/position', Twist, timeout=100)
            data_odom = rospy.wait_for_message('/unity_command/ground_truth/husky/odom', Odometry, timeout=100)
            self.assignOdomCoords(data_odom)
            self.target_callback(data_pos)
...
def main():
    ...
    subOdom = rospy.Subscriber("/unity_command/ground_truth/husky/odom", Odometry, config.assignOdomCoords)
    subLaser = rospy.Subscriber("/husky/scan", LaserScan, obs.assignObs, config)
    TotalDistance = rospy.Subscriber("/total_distance",Float32, getDistance)

    # env_state_pub = rospy.Subscriber("/env_state_pub",Twist, config.getEnvState)

    subGoal = rospy.Subscriber('/target/position', Twist, config.target_callback)

    # data_pos = rospy.wait_for_message('/target/position', Twist, timeout=100)
    # config.target_callback(data_pos)
    ...
    while not rospy.is_shutdown():
        ...
        if (atGoal(config,x) == False): # Not reached the goal
